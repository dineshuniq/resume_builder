    ...
    <div className="w-[210mm] min-h-[297mm] bg-gray-50 p-[15mm] font-sans text-gray-800">
      ...
      <div className="relative mb-10 h-[35mm]">
        ...
        <div className="absolute top-[10mm] left-[15mm] bg-white p-6 rounded-lg shadow-xl">
          ...
          <p className="text-sm text-purple-600">{renderBoldText(personalInfo.title)}</p>
        </div>
        <div className="absolute top-[5mm] right-0 flex flex-col gap-2 text-xs text-gray-600">
          {personalInfo.email && <span className="flex items-center gap-1 bg-white px-3 py-2 rounded shadow"><Mail size={12} /> {renderBoldText(personalInfo.email)}</span>}
          {personalInfo.phone && <span className="flex items-center gap-1 bg-white px-3 py-2 rounded shadow"><Phone size={12} /> {renderBoldText(personalInfo.phone)}</span>}
          {personalInfo.location && <span className="flex items-center gap-1 bg-white px-3 py-2 rounded shadow"><MapPin size={12} /> {renderBoldText(personalInfo.location)}</span>}
          {personalInfo.linkedin && <span className="flex items-center gap-1 bg-white px-3 py-2 rounded shadow"><Linkedin size={12} /> {renderBoldText(personalInfo.linkedin)}</span>}
        </div>
      </div>
      ...
      <div className="flex gap-6">
        <div className="flex-1">
          {experience.length > 0 && (
            <div className="mb-8">
              <h2 className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-4">{getSectionTitle(data, "experience", "Experience")}</h2>
        ...
        <div className="w-[55mm]">
          {skills.length > 0 && (
            <div className="mb-6 bg-white p-5 rounded-lg shadow-sm">
              <h2 className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-3">{getSectionTitle(data, "skills", "Skills")}</h2>
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <span key={skill.id} className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded">{renderBoldText(skill.name)}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {education.length > 0 && (
            <div className="mb-6 bg-white p-5 rounded-lg shadow-sm -mt-3 ml-2">
              <h2 className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-3">{getSectionTitle(data, "education", "Education")}</h2>
              {education.map((edu) => (
          ...
          {languages.length > 0 && (
            <div className="bg-white p-5 rounded-lg shadow-sm -mt-3 ml-4">
              <h2 className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-3">{getSectionTitle(data, "languages", "Languages")}</h2>
              {languages.map((lang) => (
