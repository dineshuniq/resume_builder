    ...
    <div className="w-[210mm] min-h-[297mm] bg-white font-sans text-gray-800 overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-[15mm] pb-12 text-white" style={{ clipPath: "polygon(0 0, 100% 0, 100% 85%, 0 100%)" }}>
          ...
          <p className="text-lg opacity-80">{renderBoldText(personalInfo.title)}</p>
        ...
        <div className="p-[15mm] -mt-6">
          ...
          <div className="flex gap-8">
            <div className="flex-1">
              {experience.length > 0 && (
                <div className="mb-8">
                  <h2 className="text-lg font-bold text-indigo-600 mb-4">{getSectionTitle(data, "experience", "Experience")}</h2>
                  {experience.map((exp) => (
                    <div key={exp.id} className="mb-6">
                      ...
                      <p className="text-sm text-indigo-500 mb-2">{renderBoldText(exp.company)}</p>
            ...
            <div className="w-[55mm]">
              {skills.length > 0 && (
                <div className="mb-6 bg-purple-50 p-5 rounded-lg">
                  <h2 className="text-sm font-bold text-purple-600 mb-3">{getSectionTitle(data, "skills", "Skills")}</h2>
                  <div className="flex flex-wrap gap-2">
                    {skills.map((skill) => (
                      <span key={skill.id} className="text-xs bg-white text-purple-700 px-2 py-1 rounded shadow-sm">{renderBoldText(skill.name)}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {education.length > 0 && (
                <div className="mb-6 bg-indigo-50 p-5 rounded-lg">
                  <h2 className="text-sm font-bold text-indigo-600 mb-3">{getSectionTitle(data, "education", "Education")}</h2>
                  {education.map((edu) => (
              ...
              {languages.length > 0 && (
                <div className="bg-pink-50 p-5 rounded-lg">
                  <h2 className="text-sm font-bold text-pink-600 mb-3">{getSectionTitle(data, "languages", "Languages")}</h2>
                  {languages.map((lang) => (
