            ...
            <div className="mb-8">
              ...
              {experience.map((exp) => (
                <div key={exp.id} className="mb-6 border-l-4 border-blue-600 pl-4">
                  ...
                  <p className="text-sm text-blue-600 mb-1">{renderBoldText(exp.company)} | {exp.from} - {exp.to}</p>
          ...
          {skills.length > 0 && (
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <span key={skill.id} className="text-xs bg-green-100 text-green-700 px-2 py-1">{renderBoldText(skill.name)}</span>
                ))}
              </div>
            </div>
          )}
