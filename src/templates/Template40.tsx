      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 bg-gradient-to-br from-cyan-400 to-blue-500 rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-lg">
            {stripBoldMarkers(personalInfo.fullName).charAt(0)}
          </div>
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-1">{renderBoldText(personalInfo.fullName)}</h1>
            <p className="text-sm text-blue-500 font-medium">{renderBoldText(personalInfo.title)}</p>
          </div>
        </div>
        <div className="flex flex-wrap gap-4 mt-6 text-sm text-gray-500 border-t border-gray-100 pt-4">
          {personalInfo.email && <span className="bg-gray-50 px-3 py-1 rounded-full">{renderBoldText(personalInfo.email)}</span>}
          {personalInfo.phone && <span className="bg-gray-50 px-3 py-1 rounded-full">{renderBoldText(personalInfo.phone)}</span>}
          {personalInfo.location && <span className="bg-gray-50 px-3 py-1 rounded-full">{renderBoldText(personalInfo.location)}</span>}
          {personalInfo.linkedin && <span className="bg-gray-50 px-3 py-1 rounded-full">{renderBoldText(personalInfo.linkedin)}</span>}
        </div>
      </div>
            ...
            <div className="bg-white rounded-2xl shadow-md p-6 mb-6">
              ...
              {experience.map((exp) => (
                <div key={exp.id} className="mb-5">
                  ...
                  <p className="text-sm text-blue-500 mb-2">{renderBoldText(exp.company)} | {renderBoldText(exp.location)}</p>
          ...
          {skills.length > 0 && (
            <div className="bg-white rounded-2xl shadow-md p-5 mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <span key={skill.id} className="text-xs bg-gradient-to-r from-cyan-50 to-blue-50 text-blue-600 px-3 py-1.5 rounded-full shadow-sm">{renderBoldText(skill.name)}</span>
                ))}
              </div>
            </div>
          )}
            ...
            <div className="bg-white rounded-2xl shadow-md p-5">
              ...
              {languages.map((lang) => (
                <div key={lang.id} className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-xs text-gray-600">{renderBoldText(lang.name)}</p>
                </div>
              ))}
            </div>
          )}
