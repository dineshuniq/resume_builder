    ...
    <div className="w-[210mm] min-h-[297mm] font-sans text-gray-800">
      ...
      <div className="bg-[#c0392b] p-4 text-white text-sm">
        <div className="flex flex-wrap gap-6">
          ...
          {personalInfo.linkedin && <span>{renderBoldText(personalInfo.linkedin)}</span>}
        </div>
      </div>
      ...
      <div className="p-[15mm]">
        {summary && (
          <div className="mb-8">
            <h2 className="text-lg font-bold text-[#e74c3c] mb-3">{getSectionTitle(data, "summary", "Summary")}</h2>
            ...
            {experience.length > 0 && (
              <div className="mb-8">
                <div className="bg-[#3498db] text-white p-3 mb-4 -mx-[15mm] px-[15mm]">
                  <h2 className="text-lg font-bold">{getSectionTitle(data, "experience", "Experience")}</h2>
                </div>
                {experience.map((exp) => (
                  <div key={exp.id} className="mb-6">
                    ...
                    <p className="text-sm text-[#3498db] mb-2">{renderBoldText(exp.company)} | {renderBoldText(exp.location)}</p>
                    {exp.projects.map((project) => (
                      <div key={project.id} className="mb-2">
                        <p className="text-sm font-semibold text-gray-700">{renderBoldText(project.title)}</p>
            ...
            {skills.length > 0 && (
              <div className="mb-8">
                <div className="bg-[#2ecc71] text-white p-3 mb-4 -mx-0">
                  <h2 className="text-sm font-bold">{getSectionTitle(data, "skills", "Skills")}</h2>
                </div>
                <div className="flex flex-wrap gap-2">
                  {skills.map((skill) => (
                    <span key={skill.id} className="text-xs bg-[#2ecc71] text-white px-3 py-1">{renderBoldText(skill.name)}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {education.length > 0 && (
              <div className="mb-8">
                <div className="bg-[#9b59b6] text-white p-3 mb-4">
                  <h2 className="text-sm font-bold">{getSectionTitle(data, "education", "Education")}</h2>
                </div>
                {education.map((edu) => (
            ...
            {languages.length > 0 && (
              <div>
                <div className="bg-[#f39c12] text-white p-3 mb-4">
                  <h2 className="text-sm font-bold">{getSectionTitle(data, "languages", "Languages")}</h2>
                </div>
                {languages.map((lang) => (
