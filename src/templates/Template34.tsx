            ...
            <div className="bg-white p-6 shadow-lg mb-4" style={{ transform: "rotateY(-2deg)", transformStyle: "preserve-3d" }}>
              ...
              {experience.map((exp) => (
                <div key={exp.id} className="mb-5">
                  ...
                  <p className="text-sm text-gray-500 mb-2">{renderBoldText(exp.company)}</p>
          ...
          {skills.length > 0 && (
            <div className="bg-white p-5 shadow-lg mb-4" style={{ transform: "rotateY(3deg)", transformStyle: "preserve-3d" }}>
              ...
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <span key={skill.id} className="text-xs bg-gray-100 px-2 py-1">{renderBoldText(skill.name)}</span>
                ))}
              </div>
            </div>
          )}
