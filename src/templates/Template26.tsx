  ...
  return (
    <div className="w-[210mm] min-h-[297mm] bg-gray-100 p-[15mm] font-sans text-gray-800">
      ...
      <div className="bg-indigo-600 text-white p-6 rounded-lg shadow-lg mb-6">
        <h1 className="text-3xl font-bold mb-1">{renderBoldText(personalInfo.fullName)}</h1>
        <p className="text-sm opacity-80">{renderBoldText(personalInfo.title)}</p>
      </div>
            ...
            <div className="bg-white p-5 rounded-lg shadow-sm mb-4">
              ...
              {experience.map((exp) => (
                <div key={exp.id} className="mb-5">
                  ...
                  <p className="text-sm text-indigo-500 mb-2">{renderBoldText(exp.company)} | {renderBoldText(exp.location)}</p>
          ...
          {skills.length > 0 && (
            <div className="bg-white p-5 rounded-lg shadow-sm mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <span key={skill.id} className="text-xs bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full">{renderBoldText(skill.name)}</span>
                ))}
              </div>
            </div>
          )}
