  ...
  return (
    <div className="w-[210mm] min-h-[297mm] bg-white p-[15mm] font-sans text-gray-800">
      ...
      <div className="mb-8">
        ...
        <p className="text-lg text-gray-500 font-light mb-4">{renderBoldText(personalInfo.title)}</p>
      ...
      {summary && (
        <div className="mb-8">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">{getSectionTitle(data, "summary", "Professional Summary")}</h2>
      ...
      {experience.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-4">{getSectionTitle(data, "experience", "Experience")}</h2>
          {experience.map((exp) => (
      ...
      {education.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-4">{getSectionTitle(data, "education", "Education")}</h2>
          {education.map((edu) => (
            <div key={edu.id} className="mb-3">
              ...
              <p className="text-sm text-gray-500">{renderBoldText(edu.institution)} <span className="text-gray-300">|</span> {renderBoldText(edu.score)}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {skills.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">{getSectionTitle(data, "skills", "Skills")}</h2>
          <div className="flex flex-wrap gap-2">
            {skills.map((skill) => (
              <span key={skill.id} className="text-xs bg-gray-50 text-gray-600 px-3 py-1 rounded-full">{renderBoldText(skill.name)}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {languages.length > 0 && (
        <div className="mb-8">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">{getSectionTitle(data, "languages", "Languages")}</h2>
          <div className="flex flex-wrap gap-4 text-sm">
            {languages.map((lang) => (
              <span key={lang.id} className="text-gray-600">{renderBoldText(lang.name)} <span className="text-gray-400">({lang.proficiency})</span></span>
            ))}
          </div>
        </div>
      )}
      ...
      {certifications.length > 0 && (
        <div>
          <h2 className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3">{getSectionTitle(data, "certifications", "Certifications")}</h2>
          {certifications.map((cert) => (
            <div key={cert.id} className="text-sm text-gray-600 mb-1">
              {renderBoldText(cert.name)} <span className="text-gray-400">- {renderBoldText(cert.organization)}, {cert.year}</span>
            </div>
          ))}
        </div>
      )}
